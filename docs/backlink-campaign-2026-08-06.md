...
- Completed external public listings: 2 public articles; three external contributions are awaiting maintainer review
- Published source surfaces: 3 GitHub repository / release pages
- Unique published referring root domains: 3 (`github.com` source surfaces; `dev.to` and `hashnode.dev` articles)
- Follow + indexable external listings: 0 completed; 3 expected after review and deployment
- Nofollow / UGC listings: 4 public pages (three GitHub source pages and the Hashnode article); DEV.to's anchors are not `rel=nofollow`, but that page itself currently has `noindex,nofollow`
- Noindex listings: 1 (DEV.to article)
- Pending: 3 external contributions
- Ready for user-owned publication: 1 AUR source package with passing Arch CI
...
| Arch User Repository | aur.archlinux.org | https://aur.archlinux.org/packages/zoxide-doctor | Installable `zoxide-doctor` package | zoxide installation diagnostics | Platform-generated package URL | https://zoxide.org/tools/zoxide-doctor/ | Package URL field | Expected followable; a representative package's URL and package-name links have no blocking `rel` | Representative page HTTP 200 and indexable; final page audit still required | Ready for user submission | Official Arch and AUR searches returned no `zoxide-doctor` package. Arch CI passed `makepkg`, `.SRCINFO` comparison, package installation, and `zoxide-doctor --help`; submit after AUR account and SSH-key setup |
...
- The AUR package is not counted until it is pushed to the user-owned AUR namespace and the public page is audited. Its template and CI are ready; the remaining account and SSH public-key setup is documented in `docs/backlink-assets/aur-zoxide-doctor-submission-2026-08-08.md`.
